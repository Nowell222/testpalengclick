...
import {
  ...
  History, TrendingUp, Calendar, ChevronDown, ChevronUp,
} from "lucide-react";
...
const VendorDashboardHome = () => {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Pay Card — pulled up on mobile immediately after status banner */}
      ...
      {/* QR Code — prominent on mobile directly after pay card */}
      <div className="rounded-2xl border bg-card p-5 shadow-civic flex flex-col sm:flex-row items-center gap-5">
        <div className="flex flex-col items-center text-center sm:items-start sm:text-left flex-1">
          <p className="text-xs font-medium uppercase tracking-wider text-muted-foreground mb-1">Your Stall QR Code</p>
          <p className="font-semibold text-foreground text-sm">Stall {stall?.stall_number}</p>
          <p className="text-xs text-muted-foreground mt-0.5">Show this to the cashier for payment processing</p>
          <p className="text-xs font-mono text-muted-foreground mt-2 break-all leading-relaxed">{vendor?.qr_code}</p>
        </div>
        <div className="rounded-xl border-2 border-dashed border-border p-3 bg-white shrink-0">
          {vendor?.qr_code
            ? <QRCodeSVG value={vendor.qr_code} size={110} level="H" />
            : <QrCode className="h-16 w-16 text-muted-foreground/30" />}
        </div>
      </div>

      {/* KPI Stats — larger values on mobile */}
      ...
      {/* Quick Links — collapsible on mobile to reduce scroll depth */}
          ...
          <div className="border-t divide-y">
            {[
              ...
              <Link
                key={l.to}
                to={l.to}
                className="flex items-center justify-between px-5 py-3 text-sm text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors"
              >
