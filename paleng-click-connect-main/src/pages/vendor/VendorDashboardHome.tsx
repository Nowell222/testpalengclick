    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col gap-2 sm:gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {d.unreadNotifs.length > 0 && (
          <Link to="/vendor/notifications">
            <div className="flex items-center gap-1.5 sm:gap-2 rounded-lg sm:rounded-xl border border-primary/20 bg-primary/5 px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm text-primary hover:bg-primary/10 transition-colors whitespace-nowrap">
              <Bell className="h-3.5 sm:h-4 w-3.5 sm:w-4 shrink-0" />
              <span className="hidden xs:inline">{d.unreadNotifs.length} unread notification{d.unreadNotifs.length > 1 ? "s" : ""}</span>
              <span className="xs:hidden">{d.unreadNotifs.length}</span>
              <ArrowRight className="h-3 sm:h-3.5 w-3 sm:w-3.5 shrink-0" />
            </div>
          </Link>
        )}
      </div>
      ...
      {d.isCurrentMonthPaid ? (
        <motion.div
          ...
          className="flex flex-col sm:flex-row sm:items-center sm:justify-between rounded-lg sm:rounded-2xl border border-success/20 bg-success/5 px-3 sm:px-5 py-3 sm:py-4 gap-2 sm:gap-0"
        >
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex h-7 sm:h-9 w-7 sm:w-9 items-center justify-center rounded-full bg-success/15 shrink-0">
              <CheckCircle2 className="h-4 sm:h-5 w-4 sm:w-5 text-success" />
            </div>
            <div className="min-w-0">
              <p className="text-xs sm:text-sm font-semibold text-success">{MONTHS[currentMonth - 1]} {d.currentYear} — Paid ✓</p>
              <p className="text-xs text-muted-foreground">Your stall fee for this month is fully settled</p>
            </div>
          </div>
          {!d.allPaid && d.nextUnpaidMonth <= 12 && (
            <Link to="/vendor/pay">
              <Button size="sm" variant="outline" className="border-success/30 text-success hover:bg-success/10 shrink-0 text-xs">
                Pay {MONTHS[d.nextUnpaidMonth - 1]} early
                <ArrowRight className="ml-1 sm:ml-1.5 h-2.5 sm:h-3 w-2.5 sm:w-3" />
              </Button>
            </Link>
          )}
        </motion.div>
        ...
        <motion.div
          initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
          className="flex flex-col sm:flex-row sm:items-center sm:justify-between rounded-lg sm:rounded-2xl border border-accent/20 bg-accent/5 px-3 sm:px-5 py-3 sm:py-4 gap-2 sm:gap-0"
        >
          <div className="flex items-center gap-2 sm:gap-3">
            <div className="flex h-7 sm:h-9 w-7 sm:w-9 items-center justify-center rounded-full bg-accent/15 shrink-0">
              <AlertCircle className="h-4 sm:h-5 w-4 sm:w-5 text-accent" />
            </div>
            <div className="min-w-0">
          ...
          <Link to="/vendor/pay">
            <Button size="sm" variant="hero" className="shrink-0 text-xs w-full sm:w-auto">
              Pay Now <ArrowRight className="ml-1 sm:ml-1.5 h-2.5 sm:h-3 w-2.5 sm:w-3" />
            </Button>
          </Link>
        </motion.div>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:grid-cols-4">
        {[
          ...
          <div key={c.label} className="rounded-lg sm:rounded-2xl border bg-card p-2.5 sm:p-4 shadow-civic">
            <div className="flex items-center justify-between mb-1.5 sm:mb-2">
              <p className="text-[10px] sm:text-xs text-muted-foreground uppercase tracking-wider truncate">{c.label}</p>
              <div className={`flex h-5 sm:h-7 w-5 sm:w-7 items-center justify-center rounded-md sm:rounded-lg shrink-0 ${c.bg}`}>
                <c.icon className={`h-2.5 sm:h-3.5 w-2.5 sm:w-3.5 ${c.color}`} />
              </div>
            </div>
            <p className={`font-mono text-sm sm:text-lg font-bold ${c.color} truncate`}>{c.value}</p>
      ...
      {/* ── Main content row ─────────────────────────────────────────────── */}
      <div className="grid gap-4 sm:gap-5 lg:grid-cols-[1fr_280px]">

        {/* LEFT: Chart + recent payments */}
        <div className="space-y-4 sm:space-y-5">

          {/* Payment progress chart */}
          <div className="rounded-lg sm:rounded-2xl border bg-card p-3 sm:p-5 shadow-civic">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 gap-2">
              <div className="min-w-0">
                <h3 className="text-sm sm:text-base font-semibold text-foreground">Payment Progress — {d.currentYear}</h3>
                <p className="text-xs text-muted-foreground">Monthly paid (teal) vs amount due (gray)</p>
              </div>
              <Link to="/vendor/statement">
                <Button variant="ghost" size="sm" className="text-primary h-7 text-xs gap-1 shrink-0 px-2">
            ...
            {d.chartData.length === 0 ? (
              ...
              <ResponsiveContainer width="100%" height={160}>
                <BarChart data={d.chartData} barGap={2} barSize={12}>
                  <CartesianGrid strokeDasharray="3 3" stroke="hsl(220,13%,91%)" vertical={false} />
                  <XAxis dataKey="month" tick={{ fontSize: 10, fill: "hsl(220,10%,55%)" }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 9, fill: "hsl(220,10%,55%)" }} tickFormatter={v => v >= 1000 ? `₱${v/1000}k` : `₱${v}`} axisLine={false} tickLine={false} width={40} />
                  <Tooltip
                    formatter={(v: number, name: string) => [fmt(v), name === "paid" ? "Paid" : "Due"]}
                    contentStyle={{ borderRadius: "10px", border: "1px solid hsl(220,13%,88%)", fontSize: "11px" }}
                  />
          ...
          {/* Recent payments */}
          <div className="rounded-lg sm:rounded-2xl border bg-card shadow-civic">
            <div className="flex items-center justify-between border-b px-3 sm:px-5 py-2.5 sm:py-3.5">
              <h3 className="text-sm sm:text-base font-semibold text-foreground">Recent Payments</h3>
              <Link to="/vendor/history">
                <Button variant="ghost" size="sm" className="text-primary h-7 text-xs gap-1 px-2">
            ...
            <div className="divide-y">
              {d.payments.map((p: any) => (
                <div key={p.id} className="flex items-center gap-2 sm:gap-3 px-3 sm:px-5 py-2 sm:py-3 hover:bg-secondary/30 transition-colors">
                  <div className={`flex h-7 sm:h-8 w-7 sm:w-8 shrink-0 items-center justify-center rounded-full ${
                    p.status === "completed" ? "bg-success/10" :
                    p.status === "pending"   ? "bg-amber-100"  : "bg-accent/10"
                  }`}>
                    {p.status === "completed"
                      ? <CheckCircle2 className="h-3.5 sm:h-4 w-3.5 sm:w-4 text-success" />
                      : p.status === "pending"
                      ? <Clock className="h-3.5 sm:h-4 w-3.5 sm:w-4 text-amber-600" />
                      : <AlertCircle className="h-3.5 sm:h-4 w-3.5 sm:w-4 text-accent" />}
                  </div>
                  ...
                  <div className="text-right shrink-0">
                    <p className="font-mono text-xs sm:text-sm font-bold text-foreground">{fmt(Number(p.amount))}</p>
              ...
              {d.payments.length === 0 && (
                <div className="flex flex-col items-center justify-center py-8 sm:py-10 gap-2 text-muted-foreground">
                  <CreditCard className="h-6 sm:h-8 w-6 sm:w-8 opacity-30" />
                  <p className="text-xs sm:text-sm">No payments recorded yet</p>
                  <Link to="/vendor/pay">
                    <Button size="sm" variant="outline" className="mt-1 text-xs">Make your first payment</Button>
                  </Link>
                </div>
              )}
            </div>
        ...
        <div className="space-y-3 sm:space-y-4">
          ...
          {/* Pay card */}
          <motion.div whileHover={{ y: -2 }} className="rounded-lg sm:rounded-2xl border bg-card p-3 sm:p-5 shadow-civic">
            <p className="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2 sm:mb-3">
              {d.isCurrentMonthPaid ? "Next Bill" : "Amount Due"}
            </p>
            <p className="font-mono text-2xl sm:text-3xl font-bold text-foreground leading-none">
            ...
            {d.paidThisMonth > 0 && !d.isCurrentMonthPaid && (
              <div className="mt-2 sm:mt-3 space-y-1">
                <div className="flex justify-between text-xs text-muted-foreground">
                  ...
                  <span className="text-xs">{fmt(d.paidThisMonth)}</span>
                </div>
                ...
                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Remaining</span>
                  <span className="text-accent font-medium text-xs">{fmt(d.remainingThisMonth)}</span>
                </div>
              </div>
            )}
            <Link to="/vendor/pay" className="block mt-2.5 sm:mt-4">
              <Button variant="hero" size="lg" className="w-full text-xs sm:text-sm">
                <CreditCard className="mr-1.5 sm:mr-2 h-3 sm:h-4 w-3 sm:w-4" />
          ...
          {/* QR Code */}
          <div className="rounded-lg sm:rounded-2xl border bg-card p-3 sm:p-5 shadow-civic flex flex-col items-center text-center">
            <p className="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2 sm:mb-3">Your Stall QR Code</p>
            <div className="rounded-lg sm:rounded-xl border-2 border-dashed border-border p-2 sm:p-3 bg-white">
              {vendor?.qr_code
                ? <QRCodeSVG value={vendor.qr_code} size={100} level="H" />
                : <QrCode className="h-12 sm:h-16 w-12 sm:w-16 text-muted-foreground/30" />}
            </div>
            <p className="mt-1.5 sm:mt-2.5 font-semibold text-foreground text-xs sm:text-sm">Stall {stall?.stall_number}</p>
            <p className="text-[10px] sm:text-xs font-mono text-muted-foreground mt-0.5 break-all leading-relaxed">{vendor?.qr_code}</p>
          </div>

          {/* Quick links */}
          <div className="rounded-lg sm:rounded-2xl border bg-card p-3 sm:p-4 shadow-civic">
            <p className="text-[10px] sm:text-xs font-medium uppercase tracking-wider text-muted-foreground mb-2 sm:mb-3">Quick Links</p>
            <div className="space-y-0.5">
              {[
                ...
                <Link
                  key={l.to}
                  to={l.to}
                  className="flex items-center justify-between rounded-lg px-2 sm:px-3 py-1.5 sm:py-2.5 text-xs sm:text-sm text-muted-foreground hover:bg-secondary hover:text-foreground transition-colors"
                >
                  <div className="flex items-center gap-1.5 sm:gap-2.5 min-w-0">
                    <l.icon className="h-3.5 sm:h-4 w-3.5 sm:w-4 shrink-0" />
                    <span className="truncate">{l.label}</span>
                  </div>
                  <div className="flex items-center gap-1 shrink-0">
                    {l.badge ? (
                      <span className="flex h-4 w-4 items-center justify-center rounded-full bg-primary text-[8px] sm:text-[10px] font-bold text-primary-foreground shrink-0">
                        {l.badge}
                      </span>
                    ) : null}
                    <ArrowRight className="h-2.5 sm:h-3.5 w-2.5 sm:w-3.5 opacity-40 shrink-0" />
                  </div>
                </Link>
              ))}
            </div>
